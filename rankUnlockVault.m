function recoveredValidData = unlockVault(vault,unlockingSet,DEGREE,n1,id,noOfSolution)
% unlockVault()
...
addpath('./sta/');
addpath('./data/');
addpath('./Murty/Murty k-best assignment/');
...
%allPValues
%count how many pairs has p-higher than 0.0
numel(find(allPValues>0))
...
%costMatrix
opt_fun = @munkres;
assignment_list=k_best_assign(-allPValues,noOfSolution,opt_fun);
%[costMatrix', assignment_list{1}(:,2)]
...
[~,idx] = sort(assignment_list{solution}(:,2)); % sort just the first column
sortedmat = assignment_list{solution}(idx,:); % sort the whole matrix using the sort indices
